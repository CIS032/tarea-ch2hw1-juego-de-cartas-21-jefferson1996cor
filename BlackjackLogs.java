...
public class BlackjackLogs {
    ...
    static boolean jugarBlackjack() {
         ...
         */
        String aux1 = "";
        if (manoRepartidor.getBlackjackValue() == 21) {
        ...
        while (true) {
            ...
            aux3 += "Tus cartas son:\n";
            for (int i = 0; i < manoUsuario.getCardCount(); i++) {
                aux3 += manoUsuario.getCarta(i) + "\n";
            }
            aux3 += "Su total es: " + manoUsuario.getBlackjackValue();
            ...
            if (userAction.equalsIgnoreCase("e")) {
                ...
                JOptionPane.showMessageDialog(null, "Su total es ahora " + manoUsuario.getBlackjackValue());
                guarda+= "El usuario acierta"+"\nSu tarjeta es la \n" + newCard+"Su total es ahora " + manoUsuario.getBlackjackValue();
                if (manoUsuario.getBlackjackValue() > 21) {
        ...
        while (manoRepartidor.getBlackjackValue() <= 16) {
            Carta newCard = baraja.cantidadCartas();
        ...
        JOptionPane.showMessageDialog(null, "El total del distribuidor es " + manoRepartidor.getBlackjackValue());
        ...
        if (manoRepartidor.getBlackjackValue() == manoUsuario.getBlackjackValue()) {
            JOptionPane.showMessageDialog(null, "El distribuidor gana en un empate. Pierdes.");
            return false;
        } else if (manoRepartidor.getBlackjackValue() > manoUsuario.getBlackjackValue()) {
            JOptionPane.showMessageDialog(null, "El distribuidor gana, " + manoRepartidor.getBlackjackValue()
                    + "apunta a " + manoUsuario.getBlackjackValue() + ".");
            return false;
        } else {
            JOptionPane.showMessageDialog(null,
                    "Usted gana, " + manoUsuario.getBlackjackValue()
                    + "apunta a " + manoRepartidor.getBlackjackValue() + ".");
            return true;
        }
